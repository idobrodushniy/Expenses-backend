...
class UserSerializer(serializers.ModelSerializer):
    ...
    def update(self, instance, validated_data):
        ...
        if validated_data.get('username') is not None and validated_data.get('username') != "":
            instance.username = validated_data['username']
        if validated_data.get('email') is not None and validated_data.get('email') != "":
            instance.email = validated_data['email']
        if validated_data.get('groups') is not None and validated_data.get('groups') != []:
